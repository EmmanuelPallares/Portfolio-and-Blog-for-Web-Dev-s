...
export async function getSeosProps() {
  const SEOSQUERY = gql`
    query {
      blogPosts {
        seos
  ...
  `;
  return await graphcms.request(SEOSQUERY);
}

export async function getEachSeoBlog(seo) {
  const SEOINDIVIDUAL = gql`
    query SEOINDIVIDUAL { $seo: String! }{
      blogPosts(where: { seo: $seo }) {
        title
        coverImage {
          url
        }
  ...
  return await graphcms.request(SEOINDIVIDUAL, variables);
}
